...
class TagSchema {
  ...
  async findUserTags(_id) {
    try {
      ...
      const x = readdirSync(`${userDirectory}/${_id}/tags`).reduce(
        (acc, cur, i) =>
          acc +
          `${i == 0 ? "" : ","}` +
          readFileSync(path.join(`${userDirectory}/${_id}/tags`, `/${cur}`), { encoding: "utf8" }),
        "["
      );
      const y = `${x}]`;
  ...
  async findById({ _id, userId }) {
    try {
      const x = readFileSync(path.join(userDirectory, `/${userId}/tags/${_id}.txt`), {
        encoding: "utf8",
      });
